...
import { FileText, Calendar, CreditCard as Edit2, Check, X, CircleAlert as AlertCircle, ChevronLeft, ChevronRight, Eye, RefreshCw } from 'lucide-react';
...
interface DocViewerProps {
  ...
  onTextSelection?: (term: string, context: string, x: number, y: number) => void;
}
  ...
  return (
    <div id={`doc-viewer-${doc.id}`} className="flex flex-col h-full bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-lg shadow-xs overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800 gap-3">
        ...
        <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-0.5 rounded-md self-start sm:self-center">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-sm transition-all duration-150 ${viewMode === 'canvas'
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-sm transition-all duration-150 ${viewMode === 'text'
                ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 shadow-xs'
      ...
      <div
        ...
        className={`flex-1 overflow-auto relative p-6 transition-all duration-300 ${flash
      ...
      >
        ...
        {viewMode === 'canvas' ? (
          ...
          <div
            className="max-w-3xl mx-auto font-sans leading-relaxed text-slate-700 dark:text-slate-300 select-text whitespace-pre-wrap selection:bg-emerald-200 dark:selection:bg-emerald-900/50"
            onMouseUp={(e) => {
              if (!onTextSelection) return;
              const selection = window.getSelection();
              if (!selection) return;
              const selectedText = selection.toString().trim();
              if (!selectedText || selectedText.length > 40) return;
              const context = getActivePageText();
              onTextSelection(selectedText, context, e.clientX, e.clientY);
